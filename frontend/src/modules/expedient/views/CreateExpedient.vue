...
import {getPathologies, saveExpedient} from "@/modules/expedient/services/service-expedient";
  ...
  methods: {
    ...
    async save(exp) {
      ...
      if (await onQuestion('Guardar expediente', '¿Está seguro de guardar este expediente?')) {
        ...
        const {data, status} = await saveExpedient(req)
        if (status === 400) {
          let message = ''
          switch (data.text) {
        ...
        if (status === 200) {
          onSuccess('Expediente creado', 'El expediente ha sido creado exitosamente').then(async () => {
